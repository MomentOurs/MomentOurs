...
public class MomentCommandController {
    ...
    public ResponseDTO<?> createMoment(@RequestBody RequestCreateMomentVO createMomentVO, @AuthenticationPrincipal CustomUserDetails user) {
        ...
        MomentDTO responseMomentDTO = momentService.createMoment(momentDTO, memberId);
        return ResponseDTO.ok(responseMomentDTO);
    }

    @PatchMapping
    public ResponseDTO<?> updateMoment(@RequestBody MomentDTO momentDTO, @AuthenticationPrincipal CustomUserDetails user) {
        log.info("수정 요청 데이터 : {}", momentDTO);
        MomentDTO responseMomentDTO = momentService.updateMoment(momentDTO, user);
        return ResponseDTO.ok(responseMomentDTO);
    }
